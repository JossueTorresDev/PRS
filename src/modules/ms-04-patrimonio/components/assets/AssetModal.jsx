...
export default function AssetModal({ isOpen, onClose, onSuccess, bien = null }) {
  ...
  // Catálogo SBN cargado desde useConfigurationData (sbnData, sbnGrupos)
  ...
  // Usar hook de configuración (incluye catálogo SBN)
  const configData = useConfigurationData();
  const { areas, categories, locations: ubicaciones, responsible: responsables, suppliers: providers, sbnData, sbnGrupos } = configData;

  // Cargar datos cuando se abre el modal
  useEffect(() => {
    if (!isOpen) return;
    configData.reload();
    if (!bien) setQrCodeEdited(false);
  }, [isOpen]);

  useEffect(() => {
    if (!bien || sbnData.length === 0) return;
    const sbnKey = bien.sbnCode || bien.codigoSbn;
    if (!sbnKey) return;
    const item = sbnData.find(d => d.codigo === sbnKey);
    if (item) {
  ...
  const handleSbnCodeChange = async (e) => {
    ...
    const item = sbnData.find(d => d.codigo === codigo) || null;
    setSbnInfo(item);
  ...
  const validateForm = () => {
    ...
    if (sbnInfo?.requiereSerieMarcaModelo) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <SelectSearch
                  ...
                  onChange={handleSbnGrupoChange}
                   options={sbnGrupos.map(g => ({ id: g, label: g }))}
                  valueKey="id"
                  labelKey="label"
                ...
                <div>
                  <SelectSearch
                    ...
                    options={sbnGrupo ? sbnData.filter(d => d.grupo === sbnGrupo).map(item => ({
                      ...item,
